...
for transaction in daily_transactions_split:
  clean = []
  for i in range(4):
    clean.append(transaction[i].strip())
...
#print("We made ${} today!".format(round(total_sales,3)))

#print(thread_sold)

thread_sold_split = []
for thread in thread_sold:
  if thread.find("&") == False:
    thread_sold_split.append(thread)
  else:
    for second_colour in thread.split('&'):
      thread_sold_split.append(second_colour)
...
for color in colors:                                
  print("\nWe sold {number} {color} threads today!".format(number = color_count(color), color = color))